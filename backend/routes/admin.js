...
router.post('/plans', isAdmin, [
  // Core fields
  body('planName').trim().notEmpty().withMessage('Plan name is required'),
  body('programTheme').trim().notEmpty().withMessage('Program Theme is required'),
  body('trainingDays').isInt({ min: 3, max: 6 }).withMessage('3-6 training days required'),
  
  // Muscle groups
  body('prioritizedMuscles')
    .isArray({ min: 1 }).withMessage('At least 1 prioritized muscle required'),
  body('neutralPoints').optional().isArray(),
  body('weakPoints').optional().isArray(),
  
  // Sessions validation
  body('sessions').isArray({ min: 1 }).withMessage('At least 1 session required'),
  body('sessions.*.exercises').isArray({ min: 1 }).withMessage('Each session needs exercises'),
